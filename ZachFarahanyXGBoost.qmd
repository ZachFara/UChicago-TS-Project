...
# This code is basically just the same as the stuff that we did in class except with RF
ts_xts <- xts(df$filled_CO, order.by = df$DateTime)
lags <- 1:168 # Number of lags to consider
lagged_data <- lapply(lags, function(l) lag(ts_xts, l))
lagged_df <- data.frame(lagged_data)
colnames(lagged_df) <- paste0("lag_", lags) # Rename columns with lag prefixes
lagged_xts <- xts(lagged_df, order.by = index(ts_xts))
lagged_xts <- na.omit(lagged_xts)
final_data <- cbind(ts_xts, lagged_xts)
...
```{r}
# Try a random XGB config to see if it might be good
params <- list(
  booster = "gbtree",
...
val_size <- 30 * 24 # Get a month of hourly data
...
# Review the results to find the best parameters
